...
const UserSettingsMenu: React.FC<UserSettingsMenuProps> = ({
  ...
  const menuItems: SettingsMenuItem[] = [
    ...
    },
    {
      id: 'about',
      label: 'Acerca de',
      description: 'Versión y información',
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <>
      <div className="relative" ref={dropdownRef}>
        ...
        {isDropdownOpen && (
          <div 
            className="absolute right-0 mt-2 w-72 sm:w-80 bg-white rounded-lg shadow-lg border border-gray-200 py-2 z-50 animate-slide-in-from-top max-w-[calc(100vw-2rem)]"
            role="menu"
                ...
                >
                  <div className="flex items-center space-x-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p id={`settings-${item.id}-desc`} className="text-xs text-gray-500 truncate">{item.description}</p>
                    </div>
                  </div>
                </button>
              ))}
